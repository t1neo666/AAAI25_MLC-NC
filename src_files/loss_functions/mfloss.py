...
class MultiFocalLoss(nn.Module):
    def __init__(self, args, gamma_neg=3, gamma_pos=1, gamma_class_ng=1.2, clip=0.05, eps=1e-8, \
        ...
        self.distribution_path = args.distribution_path
        path = args.path
        co_occurrence_matrix = torch.tensor(np.load(path, allow_pickle=True)).cuda()
...
def create_loss(gamma_neg=3, gamma_pos=0, clip=0.05, disable_torch_grad_focal_loss=True):
    return MultiFocalLoss(gamma_neg=3, gamma_pos=0, clip=0.05, disable_torch_grad_focal_loss=True)